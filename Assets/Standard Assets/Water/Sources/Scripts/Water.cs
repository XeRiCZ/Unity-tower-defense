...
public class Water : MonoBehaviour
{
    ...
    public watterSettings currentSettings = watterSettings.ultra;
    
	public WaterMode m_WaterMode = WaterMode.Refractive;
	public bool m_DisablePixelLights = true;
	public int m_TextureSize = 256;
	public float m_ClipPlaneOffset = 0.07f;
    public float reflectionCamFarClip = 200;
    public float refractionCamFarClip = 60;
    ...
    public Cubemap lowDetailReflection;
    public Cubemap lowDetailRefraction;

    public bool useOcclussionCulling = false;
	...
	public void OnWillRenderObject()
	{
        ...
        float[] newCulling = new float[32];
        newCulling[0] = 100;        // default
        newCulling[4] = 0;        // water
        newCulling[9] = newCulling[10] = newCulling[13] = newCulling[23] = 50;  // npcs
        newCulling[15] = 100;       // dynamic objects
        newCulling[25] = 0;       // wheat
        newCulling[8] = 400;        // layer
        newCulling[21] = 300;        // tree
        newCulling[19] = 200;        // terrain
        newCulling[26] = 0;        // particles

		
		Camera reflectionCamera, refractionCamera;
		CreateWaterObjects( cam, out reflectionCamera, out refractionCamera );

        reflectionCamera.useOcclusionCulling = useOcclussionCulling;
        refractionCamera.useOcclusionCulling = useOcclussionCulling;
        reflectionCamera.layerCullDistances = newCulling;
        refractionCamera.layerCullDistances = newCulling;
        ...
        bool isReflective = currentSettings == watterSettings.ultra;
        bool isRefractive = currentSettings == watterSettings.ultra || currentSettings == watterSettings.high;

		
		// Render reflection if needed
        if (isReflective)
        {
            if (mode >= WaterMode.Reflective || mode == WaterMode.Both)
            {
                ...
                reflectionCamera.farClipPlane = reflectionCamFarClip;
        ...
        {
            // Not reflective - use baked
            GetComponent<Renderer>().sharedMaterial.SetTexture("_ReflectionTex", lowDetailReflection);
        ...
        if (isRefractive)
        {
            if (mode >= WaterMode.Refractive || mode == WaterMode.Both)
            {
                ...
                refractionCamera.farClipPlane = refractionCamFarClip;
        ...
        {
            // Not refractive - use baked
            GetComponent<Renderer>().sharedMaterial.SetTexture("_RefractionTex", lowDetailRefraction);
